document.addEventListener("DOMContentLoaded", function() {
  ...
  function getCompactContexts(fieldsDict) {
    var contexts = [];
    for (var key in fieldsDict) {
      if (
        Object.prototype.hasOwnProperty.call(fieldsDict, key) &&
        key.toLowerCase().indexOf("tirada") === -1 &&
        key !== "Fórmula Sintáctica" &&
        key !== "Nombre" &&
        fieldsDict[key]
      ) {
        contexts.push({ label: key, value: fieldsDict[key] });
      }
    }
    return contexts;
  }
  ...
  function createCard(category, tag, title, fieldsDict, targetStageId) {
    ...
    card.className = "tactical-card tactical-card--compact-result";
    card.dataset.category = category;

    var compactTag = tag.indexOf("REGIÓN") === 0 ? "REGIÓN" :
      (tag.indexOf("HITO /") === 0 ? "HITO" : tag);
    var contexts = getCompactContexts(fieldsDict);
    var contextHtml = contexts.map(function(context) {
      return '<span class="card-result-separator">/</span>' +
        '<span class="card-result-label">' + context.label + ':</span>' +
        '<strong class="card-result-key" contenteditable="true">' + context.value + '</strong>';
    }).join("");

    card.innerHTML = '<div class="card-result-line">' +
      '<span class="card-type">' + compactTag + '</span>' +
      '<strong class="card-result-title" contenteditable="true">' + title + '</strong>' +
      contextHtml +
      '<button class="btn btn-pin" type="button">FIJAR</button>' +
      '<button class="btn-card-del" type="button" title="Eliminar" aria-label="Eliminar">×</button>' +
    '</div>';
  ...
  function bindEncounterChaControl(card) {
    var chaSelect = card.querySelector(".card-cha-mod");
    var raw2d6 = parseInt(card.dataset.encounterRaw2d6, 10);
    if (!chaSelect || Number.isNaN(raw2d6)) return;

    chaSelect.addEventListener("change", function(e) {
      var newCha = parseInt(e.target.value, 10) || 0;
      var newTotal = raw2d6 + newCha;
      var newReac = window.SD_DATA.ENCOUNTERS.getReaccion(newTotal);
      var attitude = card.querySelector(".card-actitud-text");
      if (attitude) attitude.innerHTML = newReac.actitud + " — " + newReac.desc;
    });
  }
  ...
  function pinToGmScreen(sourceCard, category) {
    ...
    var pinBtn = clone.querySelector(".btn-pin");
    ...
    if (clone.classList.contains("encounter-compact-card")) {
      bindEncounterChaControl(clone);
    }
  ...
  if (btnSettlement) {
    btnSettlement.onclick = function() {
      ...
      var distritosResumen = st.distritos.map(function(d) {
        var isGov = d.num === st.sedeGovIdx ? " [SEDE DE GOBIERNO]" : "";
        return '<span class="table-result-title">' + d.num + ". " + d.nombre + isGov + ':</span> ' +
          '<span class="table-result-content">' + d.puntos.join(", ") + '</span>';
      }).join('<span class="table-result-divider"> / </span>');

      createCard("mundo", "ASENTAMIENTO // " + st.tipo, st.nombre, {
        "Alineamiento": st.alineamiento,
        "Distritos & Puntos de Interés": distritosResumen,
        "Tiradas": st.rolls
  ...
  if (btnTavern) {
    btnTavern.onclick = function() {
      ...
      createCard("mundo", "TABERNA [" + tab.categoria + "]", tab.nombre, {
        "Famosa por": tab.fama,
        "Menú": tab.menu.join(", "),
        "Bebidas & Efectos": tab.bebidas.join(", "),
        "Tiradas": tab.roll
      }, "asentamientos-preview");
    };
  }
  ...
  if (btnShop) {
    btnShop.onclick = function() {
      ...
      createCard("mundo", "COMERCIO", sh.nombre, {
        "Vende": sh.tipo,
  ...
  if (btnEncounter) {
    btnEncounter.onclick = function() {
      ...
      var card = document.createElement("div");
      card.className = "tactical-card tactical-card--compact-result encounter-compact-card";
      card.dataset.category = "eventos";
      card.dataset.encounterRaw2d6 = enc.raw2d6;
      card.innerHTML = '<div class="card-result-line">' +
        '<span class="card-type">' + enc.titulo + '</span>' +
        '<strong class="card-result-title" contenteditable="true">' + enc.criatura + ' (' + enc.nivel + ')</strong>' +
        '<span class="card-result-separator">/</span>' +
        '<span class="card-result-label">Distancia:</span>' +
        '<strong class="card-result-key" contenteditable="true">' + enc.distancia + '</strong>' +
        '<span class="card-result-separator">/</span>' +
        '<span class="card-result-label">Actividad:</span>' +
        '<strong class="card-result-key" contenteditable="true">' + enc.actividad + '</strong>' +
        '<span class="card-result-separator">/</span>' +
        '<span class="card-result-label">Actitud:</span>' +
        '<strong class="card-result-key card-actitud-text" contenteditable="true">' + enc.actitud + '</strong>' +
        '<label class="card-result-cha">CHA' +
          '<select class="card-cha-mod" aria-label="Modificador de Carisma">' +
        ...
        '</label>' +
        '<span class="card-result-separator">/</span>' +
        '<span class="card-result-label">Tesoro:</span>' +
        '<strong class="card-result-key" contenteditable="true">' + enc.tesoro + '</strong>' +
        '<button class="btn btn-pin" type="button">FIJAR</button>' +
        '<button class="btn-card-del" type="button" title="Eliminar" aria-label="Eliminar">×</button>' +
      '</div>';

      bindEncounterChaControl(card);
