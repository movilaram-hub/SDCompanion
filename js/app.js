document.addEventListener("DOMContentLoaded", function() {
  ...
  // 11. RELOJES DE AMENAZA
  var btnAddClock = document.getElementById("btnAddClockBtn");
  if (btnAddClock) {
    btnAddClock.addEventListener("click", function() {
      var titleIn = document.getElementById("newClockTitle");
      var segsIn = document.getElementById("newClockSegments");
      if (!titleIn || !segsIn || !COMPONENTS || !COMPONENTS.createThreatClock) return;
      var title = titleIn.value.trim() || "Peligro Inminente";
      var clk = COMPONENTS.createThreatClock(title, parseInt(segsIn.value, 10));
      var cont = document.getElementById("threatClocksContainer");
      if (cont) cont.appendChild(clk);
      titleIn.value = "";
    });
  }

  // 12. LIMPIEZA Y PURGADO
  ...
  if (btnReset) {
    btnReset.addEventListener("click", function() {
      if (confirm("¿Vaciar todo el contenido de la Pantalla de GM y relojes activos?")) {
    ...
    loadInput.addEventListener("change", function(e) {
      ...
      reader.onload = function(evt) {
        try {
          ...
          document.querySelectorAll(".btn-clock-del").forEach(function(b) {
            b.onclick = function() { b.closest(".threat-clock-item").remove(); };
          });
        } catch (err) {
